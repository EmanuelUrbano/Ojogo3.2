...
sobreporPesonagem = {
    'npcs': {
        'home': [
            {'x': range(100, 165), 'y': range(100, 125)},
        ],
    },
}
...
def main_menu():
    ...
    while True:
        ...
        escreverLista(265, 300, 'mainMenu', 120)
...
def game():
    ...
    terrp = pygame.image.load("cara_1.png")
    terrp = pygame.transform.scale(terrp, (55, 75))
    ...
    while running:
        ...
        desenharSobre(seraqestaporcima, character, terrp, pos_x, pos_y)
        desenharBox(False)
        mainClock.tick(10)
        print(pos_y, pos_x)
        print(terrp.get_width())
        pygame.display.flip()
...
def sobreposicao(pos_x, pos_y, bd1, bd2):
    for e in range(len(sobreporPesonagem[bd1][bd2])):
        for i in sobreporPesonagem[bd1][bd2][e]['x']:
            for a in sobreporPesonagem[bd1][bd2][e]['y']:
                if pos_x == a and pos_y == i:
                    return True
    return False


def desenharBox(ativar):
    if ativar == True:
        box = pygame.Surface((largura - 200, altura/3))
        box.fill((5, 0, 205))
        screen.blit(box, (30, altura - altura/3 - 30))
        draw_text('oiiIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII',
                  font4, (225, 225, 225), screen, 40, altura - altura/3)


def escreverLista(posx, posyInicial, bd, intervalo):
    for i in range(len(funcoesEnomesDeSelecao[bd])):
        draw_text(funcoesEnomesDeSelecao[bd][i]['nome'],
                  font2, (225, 225, 225), screen, posx, posyInicial)
